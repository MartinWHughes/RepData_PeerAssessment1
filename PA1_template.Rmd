...
The mean of number of steps taken per day is `r sprintf("%.4f",steps.mean)`  
...
The interval the dataset that contains the maxiumum average number of steps is `r maxinterval`.
...
The original dataset contained `r num.missing.values` missing values.

The mean of number of steps (imputed) taken per day is `r sprintf("%.4f",steps.imputed.mean)`.
This differs from the original data by `r steps.imputed.mean - steps.mean` steps.  

The median number of steps (imputed) taken per day is `r sprintf("%.4f",steps.imputed.median)`.
This differs from the original data by `r sprintf("%.4f",steps.imputed.median - steps.median)` steps.
